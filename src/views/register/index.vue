<template>
    <div class="login_container">
        ...
        <el-form class="form" :model="signupForm" :rules="signupRules" ref="signupForms">
            <div class="form-title"><span>sign in to your</span></div>
            <div class="title-2"><span>SPACE</span></div>
            <el-form-item class="input-container" prop="userId">
                <el-input class="input-username" :prefix-icon="User" type="username" placeholder="Enter userID"
            ...
            <el-form-item class="input-container" prop="password_first">
                <el-input class="input-pwd" :prefix-icon="Lock" v-model="signupForm.password_first" show-password
                    type="password" placeholder="Enter password" />
            </el-form-item>
            <el-form-item class="input-container" prop="password_second">
                <el-input class="input-pwd" :prefix-icon="Lock" v-model="signupForm.password_second" show-password
                    type="password" placeholder="Enter password" />
            </el-form-item>
            <el-form-item>
                <el-button :loading="loading" type="submit" class="submit" @click="signup">
...
let signupForms = ref();
//收集账号与密码的数据
let signupForm = reactive({ userId: '', username: '', password_first: '', password_second: '' });
...
onMounted(() => {
    ...
    nextTick(() => {
        signupForms.value.clearValidate('userId');
        signupForms.value.clearValidate('username');
        signupForms.value.clearValidate('password_first');
        signupForms.value.clearValidate('password_second');
    });
})
const signup = async () => {
    await signupForms.value.validate()//如果表单通过校验才能提交
    loading.value = true
    // let result = await reqRegister({ username: signupForm.username, password: signupForm.password_first });
    let result = await reqRegister({
        userId: signupForm.userId,
        username: signupForm.username,
        password: signupForm.password_first,
        confirmedPassword: signupForm.password_second
    });
...
const validatorUserId = (rule: any, value: any, callback: any) => {
    if (value.length === 0) {
...
const validatorPasswordFirst = (rule: any, value: any, callback: any) => {
    if (value.length === 0) {
...
const validatorPasswordSecond = (rule: any, value: any, callback: any) => {
    if (value.length === 0) {
        callback(new Error('密码不能为空'))
    } else if (signupForm.password_first != signupForm.password_second) {
